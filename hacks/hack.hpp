		...
		**/
		for (auto player = g_game.players.begin(); player < g_game.players.end(); player++) {
			...
			if (config::show_head_tracker) {
				// ✅ CONVERTE HEAD PARA COORDENADAS DE TELA
				Vector3 headScreen = g_game.world_to_screen(&player->bones.bonePositions["head"]);

				if (headScreen.z >= 0.01f) {
					render::DrawCircle(
						g::hdcBuffer,
						headScreen.x,
						headScreen.y - width / 12,
						width / 5,
			...
			// ✅ SKELETON - AGORA FUNCIONA CORRETAMENTE
			if (config::show_skeleton_esp) {
				for (const auto& connection : boneConnections) {
					...
					Vector3 boneFromScreen = g_game.world_to_screen(&player->bones.bonePositions[boneFrom]);
					Vector3 boneToScreen = g_game.world_to_screen(&player->bones.bonePositions[boneTo]);

					// ✅ SÓ DESENHA SE ESTIVER VISÍVEL
					if (boneFromScreen.z >= 0.01f && boneToScreen.z >= 0.01f) {
						render::DrawLine(
							g::hdcBuffer,
							boneFromScreen.x, boneFromScreen.y,
							boneToScreen.x, boneToScreen.y,
							g_game.localTeam == player->team ? config::esp_skeleton_color_team : config::esp_skeleton_color_enemy
						);
					}
				}
			...
			// ✅ TUDO DAQUI EM DIANTE SÓ RENDERIZA SE EXTRA FLAGS ESTIVER ATIVO
			if (config::show_extra_flags)
			{
				...
				*/
				...
				// ✅ RASTREADOR DINÂMICO DE POSIÇÃO Y
				int flagYOffset = 0;
